...
export function useDocData() {
  ...
  const fetchDoctorInfo = async () => {
    try {
      ...
      const response = await axios.get(`${google_ngrok_url}doctor/get_doctor_info/`, {
        headers: {
  ...
  const fetchAddresses = async () => {
    try {
      ...
      const token = localStorage.getItem("Token");
      const response = await axios.get(`${google_ngrok_url}doctor/get_doctor_addresses/`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420",
        },
  ...
  const addNewAddress = async (newAddress, newAddressType) => {
    try {
      const token = localStorage.getItem("Token");
      await axios.post(`${google_ngrok_url}doctor/add_doctor_address/`, {
        address: newAddress,
  ...
  const updateDoctorInfo = async (updatedInfo) => {
    try {
      const token = localStorage.getItem("Token");
      await axios.post(`${google_ngrok_url}doctor/save_doctor_data/`, updatedInfo, {
        headers: {
          Authorization: `Bearer ${token}`,
          "ngrok-skip-browser-warning": "69420",
        },
