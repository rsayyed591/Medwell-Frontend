    ...
    fetch(`${ngrok_url}/auth/login_userdoctor/`, {
      ...
      .then(data => {
        ...
        if (data.mssg === 'Incorrect Credentials' && data.status === 0) {
          ...
          localStorage.setItem("User", JSON.stringify({ email: email, role: 'doctor' }))
          navigate("/Dashboard")
        }
      })
      .catch(err => {
        ...
        <div className="flex flex-col md:flex-row">
          <div className="bg-blue-100 p-12 md:w-1/2 relative flex flex-col justify-between">
            <div className="text-2xl text-gray-800 font-semibold mb-12">
              Welcome back, Doctor!<br />Your patients are<br />waiting for you.
            </div>
            <div className="flex items-center justify-center flex-grow">
              <div className="relative w-64 h-64">
                <div className="absolute inset-0 bg-blue-200 rounded-full"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <Stethoscope size={100} className="text-blue-600" />
                </div>
              </div>
            </div>
          </div>
          <div className="p-12 md:w-1/2">
            <h2 className="text-3xl font-bold text-center mb-8">Doctor Login</h2>
            ...
            <p className="mt-4 text-center text-sm text-gray-600">
              Don't have an account? <Link to="/doctor/signup" className="font-medium text-blue-600 hover:text-blue-500">Sign up</Link>
            </p>
          </div>
        </div>
      </div>
